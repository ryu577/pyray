...
def draw_trtmt_hist(draw, h1=100, h2=100):
    ...
    draw.polygon(
        [(41, 169), (59, 169), (59, 169 - h1), (41, 169 - h1)], fill=(255, 190, 25)
    )
...
def draw_two_gauss(
    ix=0, extrema=500, std=50, h1=0, h2=0, gap=50, alpha=0.15865525393145707
):
    im = Image.new("RGB", (512, 512), "black")
    draw = ImageDraw.Draw(im, "RGBA")
    fn = lambda x: 300 - norm.pdf(x - 250, 0, std) * 7000
...
def draw_alpha_beta_curve(draw, alpha=0.15865525393145707, effect=50, std=50):
    ...
    draw.text(
        (134 - 40, 335), "alpha= " + str(round(alpha, 2)), (255, 255, 0), font=font
    )
    draw.text(
        (134 - 40, 335 + 20), "beta= " + str(round(beta, 2)), (138, 43, 226), font=font
    )
    x1, y1 = 50, 464
...
for i in range(12):
    draw_gauss(
        i,
        std=np.sqrt(300 / (26 + 2 * i) + 2200 / (26 + 2 * i)),
        h1=50 + 4 * i,
        h2=50 + 4 * i,
    )
...
"""
for i in range(20):
    alpha = zigzag2(i / 10.11, 0.158655, 0.95, 0.01)
...
"""
for i in range(20):
    draw_two_gauss(ix=i, gap=80, std=2500 / (50 + i), h1=2 * i, h2=2 * i)
...
"""
for i in range(20):
    draw_worst_test(ix=i, gap=80, std=2500 / (50 + i), h1=2 * i, h2=2 * i)
