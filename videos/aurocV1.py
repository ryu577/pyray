...
for i in range(m.shape[0]):
    draw.ellipse(
        (m[i][0] - w, m[i][1] - w, m[i][0] + w, m[i][1] + w),
        fill=(255, 20, 102),
        outline=(0, 0, 0),
    )
    if i % 10 == 0:
    ...
    for j in range(min(int(i / 4), len(dd1))):
        ...
        if pt1[0] > pt1[1]:
            arrowV1(
                ...
                np.eye(3),
                np.array(pt1) / 100,
                np.array([sum(pt) / 2, sum(pt) / 2, 0]) / 100,
                scale=100.0,
...
for i in range(33):
    ...
    for j in range(1, min(8, int(i / 2))):
        ...
        draw.line(
            (pt_1_a[0], pt_1_a[1], pt_2_a[0], pt_2_a[1]),
            fill=(255, 255, 255, 100),
            width=4,
        )
        draw.line(
            (pt_1_b[0], pt_1_b[1], pt_2_b[0], pt_2_b[1]),
            fill=(255, 255, 255, 100),
            width=4,
        )
...
for t in range(185):
    if t % 5 == 0:
        (im, draw) = binary_classificn_pts(
            5, r=planar_rotation(-np.pi / 4), t1=180, t2=t
        )
...
for j in np.arange(38):
    (im, draw) = binary_classificn_pts(
        ...
        trnsp2=120 - 37 * 3 + j * 3,
        trnsp1=120 - 37 * 3 + j * 3,
        trp1_hi=120 + 37 * 2 - j * 2,
        trp1_lo=120 - 37 * 3 + j * 3,
        trp2_hi=120 - 37 * 3 + j * 3,
        trp2_lo=120 - 37 * 2 + j * 2,
        hi1_ind=8,
        lo1_ind=0,
        hi2_ind=13,
        lo2_ind=9,
