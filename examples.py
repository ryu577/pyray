...
def draw_wavy_sphere_accleration_wrapper(save_dir, number_of_circles):
    for i in np.arange(60):
        wavy_index = i % number_of_circles
        # 2.52 is an angle of Z axis (Why do I choose 2.52? For an aesthetic reason:))
        r = rotation(3, 2.50 + np.pi*np.sin(i/10.0) * np.random.uniform(0.8,1) / 30.0)
        ...
        # The vector that passes through the center is np.array([0,0,1])
        # Radius is oscillating randomly
        draw_sphere2(draw, np.array([0,0,0]), np.array([0,0,1]), 1, r, wavy_index = wavy_index ,num_circle = number_of_circles, rgba=(182, 183, 186, 255), width = 2)
        file_name = save_dir + str(i) + '.png'
        im.save(file_name)
...
if __name__ == "__main__":
    ...
    # With Acceleration
    draw_wavy_sphere_accleration_wrapper('Images\\RotatingOscillatingSphere\\im', 60)